...
class Panel implements IBarPanel, Doctrine\DBAL\Logging\SQLLogger
{
	...
	public function renderQueryException($e)
	{
		if ($e instanceof \PDOException && count($this->queries)) {
			...
			if ($this->connection !== NULL) {
				if (!$e instanceof \Doctrine\DBAL\DBALException || $e->connection !== $this->connection) {
					return NULL;
		...
		} elseif ($e instanceof \Doctrine\DBAL\Query\QueryException && $e->query !== NULL) {
			if ($e->query instanceof Doctrine\ORM\Query) {
				return array(
					'tab' => 'DQL',
					'panel' => $this->dumpQuery($e->query->getDQL(), $e->query->getParameters()),
				);
			} elseif ($e->query instanceof Doctrine\ORM\NativeQuery) {
				return array(
					'tab' => 'Native SQL',
	...
	public static function renderException($e, Nette\DI\Container $dic)
	{
		...
		} elseif ($e instanceof Doctrine\DBAL\Schema\SchemaException && $dic && ($em = $dic->getByType('Doctrine\Common\EventManager', FALSE))) {
			/** @var Doctrine\Common\EventManager $em */
	...
	public static function formatQuery($query, $params, array $types = array(), AbstractPlatform $platform = NULL)
	{
		...
		if (Nette\Utils\Validators::isList($params)) {
			...
			return implode('', KdybyHelpers::zipper($parts, $params));
		}
	...
	public static function highlightAnnotationLine(AnnotationException $e)
	{
		...
		if ($context['type'] === 'property') {
			...
			$line = KdybyHelpers::getPropertyLine($refl);
